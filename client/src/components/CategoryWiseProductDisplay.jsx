      ...
      <div className="relative flex items-center ">
        ...
        >
          {loading &&
            loadingCardNumber.map((_, index) => {
              return <CardLoading key={index + "CategorywiseProduct234"} />;
            })}
          {data.map((p, index) => {
            return (
              <CardProduct
                key={p._id + index + "CategorywiseProduct"}
                data={p}
              />
            );
          })}
        ...
        <div className=" w-full left-0 right-0 container mx-auto px-2 hidden lg:flex justify-between absolute">
          <button
            ...
            className="z-10 relative bg-white hover:bg-gray-100 shadow-lg text-lg p-4 rounded-full"
          >
            <FaAngleLeft />
          </button>
          <button
